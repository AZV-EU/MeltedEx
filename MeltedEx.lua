...
local REPOSITORY = {
	...
	[6888253864] = "WackyWizards.lua",
	[13541967921] = "UltimateFactoryTycoon.lua",
	
	[2474168535] = "Westbound.lua",
	
	[1340132428] = "ArmoredPatrol.lua",
	[10518166490] = "ArmoredPatrol.lua",
	[5326405001] = "BaseBattles.lua",
	...
	[15841295873] = "Minecraft.lua", -- Craft-blox 2
}
...
function _G.MX_SetupFunctionLocals(func)
	...
	env.Players = _G.SafeGetService("Players")
	env.plr = env.Players.LocalPlayer
	env.Workspace = _G.SafeGetService("Workspace")
	env.RunService = _G.SafeGetService("RunService")
	env.UserInputService = _G.SafeGetService("UserInputService")
	env.ReplicatedStorage = _G.SafeGetService("ReplicatedStorage")
	env.ContextActionService = _G.SafeGetService("ContextActionService")
...
do -- Main category
	...
	do category:BeginInline()
		category:AddButton("Tp to Cam", function()
...
if GameModule and GameModule.Init then
	task.spawn(function()
		...
		GameModule.On = true
		local env = _G.MX_SetupFunctionLocals(GameModule.Init)
		env.connections = GameModuleConnections
		env.category = category
		local f, err = pcall(GameModule.Init)
		if not f then
